...
class PindelObject():
    def __init__(self, Pindel_dir, tumor_type, file_name) -> None:
    ...
    def read(self) -> None:
        self.set_vcf_header()
        self.raw = self.read_vcf(self.file_path)
    ...
    def set_vcf_header(self) -> None:
        self.path_checkExistence(self.file_path)
        with open(self.file_path) as f:
            self.vcf_header = [l for l in f if  l.startswith('##')]

    def set_path(self) -> None:
        self.file_path = path(self.path, self.tumor_type, self.file_name + ".vcf") # open relative pindel vcf
...
""")
    ...
    for sample in samples:
        g_data = initiatePindel(ITD_data[ITD_data["sample_id"] == sample])
        Pindel = PindelObject(pindel_dir, tumor_type, sample)
